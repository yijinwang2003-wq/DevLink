...
async def test_follow_user(
    ...
    response = await client.post(f"/api/v1/users/{second_user.username}/follow", headers=auth_headers)
...
async def test_unfollow_user(
    ...
    response = await client.delete(f"/api/v1/users/{second_user.username}/follow", headers=auth_headers)
...
async def test_list_followers_and_following(
    ...
    followers_response = await client.get(f"/api/v1/users/{second_user.username}/followers")
    following_response = await client.get(f"/api/v1/users/{test_user.username}/following")
...
async def test_prevent_self_follow(
    ...
    response = await client.post(f"/api/v1/users/{test_user.username}/follow", headers=auth_headers)
